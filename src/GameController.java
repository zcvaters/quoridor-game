...
public class GameController {
	 ...
	 * 
	 * //get the 2D array of gametiles GameTile[][] tiles =
	 * GameSettings.getGameTiles();
	 * 
	 * //get the number of rows and cols on gameboard //we need this zero indexed,
	 * so subtract one. int rows = GameSettings.GetRows() - 1; int cols =
	 * GameSettings.GetCols() - 1; //get the gameTile that the player is standing on
	 * GameTile location = currentPlayer.GetTile(); //get the x, y, address of this
	 * tile int rowAddress = location.GetXCoord(); int colAddress =
	 * location.GetYCoord();
	 * 
	 * System.out.println(currentPlayer.GetName() + " is on tile (" +rowAddress+
	 * ", " +colAddress+ ")");
	 * 
	 * //does player have north? if(location.HasNorth()) {
	 * tiles[rowAddress-1][colAddress].ActivateTile(); } //does player have east?
	 * if(location.HasEast()) { tiles[rowAddress][colAddress+1].ActivateTile(); }
	 * //does player have south? if(location.HasSouth()) {
	 * tiles[rowAddress+1][colAddress].ActivateTile(); } //does player have west?
	 * if(location.HasWest()) { tiles[rowAddress][colAddress-1].ActivateTile(); }
	 * 
	 * 
	 * 
	 * }
